...
def oranla(arr):
    ...
    if(boyut==0):
        ...
        psikoloji_kontrol(mutlu,mutsuz,notr)
        resme_dok(mutlu,mutsuz,notr)

def psikoloji_kontrol(mutlu,mutsuz,notr):
    if(notr >= 50 ):
        if(mutsuz < mutlu):
            print("Bu kisinin psikolojisi gayet iyi!")
        elif(mutsuz > mutlu):
            print("Bu kisinin psikolojik sorunlari var!")
        else:
            print("Kisinin verileri anlamdirilamiyor! Lutfen baska bir kaynak ile kontrol ediiniz!")
    else:
        if(mutlu >= 95):
            print("Bu kisinin bazi sorunlari var gibi!")
        elif(mutlu <95 and mutlu >= 70):
            print("Kisi gayet saglikli gozukuyor!")
        elif(mutlu < 70 and mutlu >= 50):
            print("Kisinin psikoloji gayet iyi!")
        elif(mutlu <50 and mutlu >=40):
            print("Bu kisinin psikolojisi kotu ancak iyi arkadaslarla cozulebilecek seviyede!")
        else:
            print("Bu kisinin bir psikologa gorunmesi iyi olur!")
